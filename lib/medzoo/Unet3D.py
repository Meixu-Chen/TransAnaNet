...
import numpy as np
from lib.SpatialTransformer import ST
...
class UNet3D(BaseModel):
    ...
    def __init__(self, in_channels, n_classes, base_n_filter=4, size = [96,256,256]):#before base_n_filter=8
        ...
        self.SpatialTrans = ST.SpatialTransformer(size, mode='bilinear')#another layer of the network
    ...
    def forward(self, x):
        ...
        #print("\n Before level 5: context_4 size, out size",np.shape(out),np.shape(context_4))
        ...
        #print("\n After level 5: context_4 size, out size",np.shape(out),np.shape(context_4))
        ...
        out = self.SpatialTrans(CBCT1,DVF)
        
        #return torch.sigmoid(seg_layer)#to get values between 0 and 1

        #add spatial tranform
        return out, DVF 
