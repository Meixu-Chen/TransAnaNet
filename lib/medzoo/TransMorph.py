...
def window_partition(x, window_size):
    ...
    # print('window_partition x.shape post-op ', x.shape)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        # print('SwinTransformerBlock input x shape ', x.shape)

        shortcut = x
        x = self.norm1(x)
        x = x.view(B, H, W, T, C)
        # print('SwinTransformerBlock viewed x shape ', x.shape)
        ...
        # print('SwinTransformerBlock padded x shape ', x.shape)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        # print(in_chans, embed_dim, patch_size, patch_size)
...
class TransMorph_Model(BaseModel):
    def __init__(self, embed_dim=96, patch_size=4, in_chans=3, img_size=(32,128,128)):
        '''
        TransMorph Model
        '''
        super(TransMorph_Model, self).__init__()
        if_convskip = True
        ...
        self.spatial_trans = SpatialTransformer(img_size) #we might need to try nearest interpolation
    ...
    def forward(self, x):
        ...
        out_img = self.spatial_trans(source_img, flow)
        out_gtv = self.spatial_trans(source_gtv, flow)
        out_gtvn = self.spatial_trans(source_gtvn, flow)
        return out_img, out_gtv, out_gtvn, flow

    def test(self,device='cpu'):
        print('do nothing')  

class TransMorphML(BaseModel):
    def __init__(self, embed_dim=96, patch_size=4, in_chans=3, img_size=(32,128,128)):
        '''
        ...
        self.up4 = DecoderBlock(embed_dim // 2, reg_head_chan, skip_channels=reg_head_chan,
        ...
        self.avg_pool = nn.AvgPool3d(3, stride=2, padding=1)
        self.reg_head_s1 = RegistrationHead(in_channels=reg_head_chan, out_channels=3, kernel_size=3,)
